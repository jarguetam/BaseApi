...
namespace BaseApi.WebApi.Helpers
{
    ...
    {
        public static string NumberToText(this decimal numberAsString)
        {
            string dec;
            ...
            if (decimales > 0)
            {              
                dec = $" LPS {decimales:0,0} /100";
            }
            else
            {
                dec = $" LPS {decimales:0,0} /100";
            }
            var res = NumberToText(Convert.ToDouble(entero)) + dec;
            return res;
        }
        ...
        private static string NumberToText(double value)
        {
            ...
            else if (value < 1000000000000)
            {
                ...
                {
                    num2Text = num2Text + " " + NumberToText(value - Math.Truncate(value / 1000000) * 1000000);
                }
            }
            else if (value == 1000000000000) num2Text = "UN BILLON";
            else if (value < 2000000000000) num2Text = "UN BILLON " + NumberToText(value - Math.Truncate(value / 1000000000000) * 1000000000000);
            else
            {
                num2Text = NumberToText(Math.Truncate(value / 1000000000000)) + " BILLONES";
                if ((value - Math.Truncate(value / 1000000000000) * 1000000000000) > 0)
                {
                    num2Text = num2Text + " " + NumberToText(value - Math.Truncate(value / 1000000000000) * 1000000000000);
                }
            }
            return num2Text;
        }
